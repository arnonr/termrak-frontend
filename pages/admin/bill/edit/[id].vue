      ...
      <div class="row">
        <div class="mt-30 pl-10 pt-15 pb-10 bg-grey">
          <h4>
            ...
            <span class="ml-10">แบบฟอร์มแก้ไขใบเสร็จ</span>
          </h4>
        </div>
...
const onSubmit = async () => {
  ...
  let type_object = {
    ...
    method: "put",
    url: runtimeConfig.public.apiBase + "/bill/" + item.value.id,
  };
...
onMounted(() => {
  item.value.post_company_id = selectOptions.value.post_companies.find((x) => {
    return x.value == res.value.data.post_company_id;
  });
});
</script>
